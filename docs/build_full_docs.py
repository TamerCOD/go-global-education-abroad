...
story.append(tbl([
    ...
    ["2 · Плитки-счётчики", "Активные · Без ответа · Просрочено · В очереди · Передачи мне", "реальные цифры — не зависят от фильтров списка"],
    ["3 · «Мой день»", "Встречи, горячие, задачи, суммы в работе и прогноз", "личный план на день"],
    ["4 · Фильтры (слева)", "9 быстрых фильтров, статус, источник, страна, даты, пресеты", "один быстрый фильтр за раз; повторный клик выключает"],
...
story.append(P(
    "Девять пресетов в один клик (работают как радио-кнопки — активен один, повторный клик выключает): "
    "«Inbox 0» (требует действий), «Просроченные», «Горячие» (скоринг 60+), «Новые», «Перезвонить», «Не ответил», "
    "«Визиты в офис», «Дубли» (повторные обращения) и «Закрытые» — единственный, который показывает завершённых "
    "(выигранных и отказы); все остальные работают по открытым лидам. По умолчанию, без единого фильтра, "
    "в списке видны ВСЕ открытые лиды."))
...
story.append(tbl([
    ...
    ["Дубль", "Клиент обратился повторно — создаётся автоматически, история в оригинальном лиде", "—"],
