...
story.append(NOTE("Бот — это <b>дополнение</b> к push-уведомлениям в самой CRM, а не замена. Менеджеру push "
                  "приходит лично; в общий чат Telegram видит вся команда (полезно руководителю). А после личной "
                  "привязки (раздел «Личные уведомления сотрудникам») сотрудник получает свои события ещё и в "
                  "личку Telegram."))
...
# ── NEW ──
chapter(story, "Личные уведомления сотрудникам")
story.append(P("Кроме общего чата, бот умеет писать <b>лично</b> каждому сотруднику. Сначала сотрудник привязывает "
               "свою учётку CRM к Telegram по разовому коду — тогда свои события он получает в личку, а не теряет "
               "в общем потоке."))
story.append(P("Как привязаться", S_H2))
story += diagram("tglink", 56*mm, "Привязка: админ выдаёт код → /start → логин → код → готово")
story.append(tbl([
    ["Шаг", "Кто", "Действие"],
    ["1", "Администратор", "В админке → «Менеджеры» жмёт «📱 TG-код» у сотрудника и передаёт ему код"],
    ["2", "Сотрудник", "Открывает бота, отправляет /start"],
    ["3", "Сотрудник", "Отправляет свой логин (как при входе в CRM)"],
    ["4", "Сотрудник", "Отправляет полученный код — бот привязывает учётку"],
], [12*mm, 32*mm, None]))
story.append(NOTE("Новый код, выданный админом, <b>сбрасывает</b> прежнюю привязку (нужно привязаться заново). При "
                  "увольнении сотрудника связка стирается автоматически. Отвязаться самому — команда <b>/stop</b>."))
story.append(P("Что приходит лично", S_H2))
story.append(tbl([
    ["Событие (в личку)", "Кому"],
    ["🆕 Назначен новый лид", "менеджеру лида"],
    ["🔁 Повторное обращение клиента", "менеджеру оригинала"],
    ["✅ / ❌ Решение по согласованию этапа (+ причина при отказе)", "менеджеру, запросившему переход"],
    ["🕓 Нужно согласование этапа", "РОП / админу — лично"],
    ["⏰ Просрочен SLA по лиду", "ответственному менеджеру"],
    ["⏰ Просрочена задача", "исполнителю задачи"],
], [None, 48*mm]))
story += msg("✅ Согласование подтверждено", [
    "Лид «Айгуль Токтосунова» переведён на этап «Оплата 1» (Гульнара, РОП)."])
story += msg("❌ Переход отклонён", [
    "Этап «Оплата 1» не подтверждён (Гульнара):",
    "«чек нечитаемый, переснимите». Этап остался прежним."])
...
story.append(P("Бот работает через настройки сервера (переменные окружения Railway). Задаются один раз."))
story.append(tbl([
    ["Переменная", "Что это"],
    ["TELEGRAM_BOT_TOKEN", "токен бота от @BotFather"],
    ["TELEGRAM_CHAT_ID", "ID чата/группы для общих уведомлений команды"],
    ["PUBLIC_BASE_URL", "адрес сайта (https://goglobal.kg) — нужен для ЛИЧНОЙ привязки сотрудников"],
], [56*mm, None]))
story.append(WARN("Для <b>личных</b> уведомлений (привязка сотрудников) обязательна переменная "
                  "<b>PUBLIC_BASE_URL = https://goglobal.kg</b>: по ней бот регистрирует webhook и начинает "
                  "принимать команды /start от сотрудников. Без неё работают только общие сообщения в группу."))
